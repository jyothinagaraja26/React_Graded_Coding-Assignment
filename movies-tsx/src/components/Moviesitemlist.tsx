...
import { getDataFromServer, addToFavourites, removeFromFavourites } from "../services/Api";
...
const Moviesitemlist = (props: Props) => {

    const [filteredItems, setFilteredItems] = useState<IMovies[]>([]);
    const [toast, setToast] = useState<ToastObject[]>([]);
    const [search, setSearch] = useState<string>("");
    const [receivedData, setReceivedData] = useState<IMovies[]>([]);
    ...
    const filterItems = (searchString: string, data: IMovies[]) => {
        if (searchString !== "") {
    ...
    const handleFavourites = async (movie: IMovies) => {
        try {
        ...
        <>
            <Container>
                ...
                <ToastContainer className="p-3" style={{ zIndex: 2, position: "fixed", right: "20px", top: "20px" }}>
                    {
                        toast.map((toast, idx) => <Item key={idx} toastString={toast.toastString} toastBgType={toast.toastBgType} />)
                    }
                </ToastContainer>
                {
